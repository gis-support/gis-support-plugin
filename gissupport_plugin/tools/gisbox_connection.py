...
class GisboxConnection(QObject, Logger):
    ...
    def authenticate(self) -> bool:
        ...
        status_code = reply.attribute(QNetworkRequest.HttpStatusCodeAttribute)
        if not response_raw:
            self.message('Błąd połączenia z serwerem. Sprawdź czy adres aplikacji jest prawidłowy lub skontaktuj się z administratorem',
    ...
    def _createRequest(self, endpoint: str, content_type: str = 'application/json', with_token: bool = True) -> QNetworkRequest:
        ...
        request.setHeader(QNetworkRequest.UserAgentHeader, 'qgis_gs')
